...
xdata unchar  col_address _at_ 0X8002;           //列地址
xdata unchar  key_address _at_ 0x8001;		   //行地址
...
unint anum=0; 
unchar flag=6;				 //flag=0x00,执行默认方案；flag=0x0f,选择键，进入选择状态；flag=0x0e,确认键                 
...
void main(void)
{
  ...
  TH1=0Xd8;
  TL1=0XB0;
  while(1)		//利用zhongduan方式每10ms键盘定时扫描一次
  {	   
     functionkey();
